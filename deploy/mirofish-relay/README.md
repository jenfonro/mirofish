...
**也不再有「用量上限」**：账号一直用到窗口真正耗尽（约 100%）才被自动分配跳过。预留余量
只会让额度在重置时白白清零，而这正是上面的排序要避免的事。缓存最多 1 小时旧，所以一个
读到 99% 的账号可能实际已经用光，代价是多产生一两次 429——但 429 会被准确识别、按它指明的
窗口冷却，冷却期内不再有任何请求，所以代价是有界的，不会累积。
...
- **按需**：某个账号服务完一次请求后，若它的额度缓存超过 1 小时才读一次 `/v1/limits`，
  且只读这一个账号。刷新放在请求**之后**，不在前面——放前面会拖慢调用方，也会把上游对
  这次请求的真实判决盖掉。
