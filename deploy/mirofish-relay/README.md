...
自动调度会跳过**任一相关窗口已用满**的账号：每个请求都要看 **5 小时窗口**和 **7 天窗口**，
fable 模型还要额外看 **7 天 Fable 窗口**，取其中最紧的那个——花费同时落在这几个窗口上，所以
...
- **403 封停（反复触发限流后被上游临时封禁）**：上游的提示里直接给出解封时刻
  （`access resumes at <ISO 时间>`），**直接采用这个时刻**作为重试期限——比它早重试必然失败，
  比它晚重试则白白浪费一个已经恢复的账号。只有时间戳解析不出来时才退回 1 小时的兜底窗口。
  注意只有「封停」这一种 403 会停调：面板里手动停用账号时 relay 自己也返回 403，那不是上游判决。
