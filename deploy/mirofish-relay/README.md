...
模型 relay 还要求设备签名：**每个账号持久化一把独立的 Ed25519 密钥**，
并按「账号 × 出口」申请约 15 分钟的 device ticket，再为每个请求生成 `mrs-sig-v2` 签名（覆盖
method、pathname、时间戳、nonce、设备 ID、客户端版本，以及凭证、relay 元数据与请求体的摘要）。
私钥与账号 token 一样只进入加密凭证存储，不写入 SQLite 或日志。

**为什么按账号隔离**：官方桌面端确实是「一个安装一把密钥」，但照搬到这里是错的——一台桌面
只服务一个登录账号，而 relay 要服务几十个，安装级密钥等于告诉上游「同一台设备在驱动这几十个
账号」，这是把它们互相关联起来最直接的把手。26 个账号曾在 10 秒内被同时封停，而当时真实的
上游调用量只有每分钟 11 次——这是共享标识被识别的特征，不是限流的特征。

已有账号会**继承**原来的安装级密钥而不是立刻换掉（一整池账号同一天集体换设备本身也是信号）；
**重新登录**时才轮换成新身份，这也是账号摆脱共享标识的唯一途径；删除账号会一并删掉它的密钥，
避免别名被复用时继承旧身份。当前默认客户端标识为抓包确认的 `0.0.272`。ticket 会提前 120 秒
...
（这也是把修好的节点放回新分配的方式）；其他状态码说明有东西在中间拦截，按失败记录。

**账号与节点的绑定**：每个账号固定绑定一个节点，节点 ID 写入 SQLite，凭据存在加密配置库里。
绑定只在账号还没有节点时决定一次（按已分配数量最少优先），**之后不会再变**：请求要么走这个
出口，要么失败。

早先的版本会在节点故障或上游拒绝出口区域时把账号轮换到别的节点。那意味着一个抖动的代理就能
悄悄改掉某个账号的上游 IP，还会让两个账号共用同一个出口——正是把一批账号关联起来的特征。
现在节点坏了就是坏了：修好它（或在账号侧手动改绑），而不是让 relay 替你换一个。

- **节点网络失败**：连接超时、拒绝等传输层错误会累计失败次数，达到
  `MIROFISH_PROXY_FAILURE_THRESHOLD` 后节点停用。停用只影响**新的**绑定，已经绑在上面的账号
  仍然绑着——它们本就无处可去。测试成功即清零恢复。
- **额度类 429**（`credit_exhausted_shared`、`shared_quota_unavailable` 等）**不是节点故障**：
  这些是账号属性，换出口也不会好。自动路由会冷却该账号并换用其他账号，显式指定账号时原样
  返回错误。
...
这些错误是账号属性而非出口属性；账号的出口本来就是固定的，换节点也不会让额度回来。
...
「账号调度」的规则分配。会话在 `MIROFISH_SESSION_TTL`（默认 1800 秒）无活动后过期。
WebUI 账号表的「活跃会话」列可实时看到每个账号正在服务的窗口数。

**账号调度**：只有一种排序，没有模式可选，也没有可调参数。新会话按下面三条依次比较：

1. **7 天窗口越早重置的越先用**——那份额度不花掉就在重置时清零了，而其他账号还有一整周
   可以慢慢用。重置时间按 1 小时分档（原始时间戳永不相等，同批开的账号也差几微秒，
   直接比就会让第一条决出全部胜负，后面两条形同虚设）；超过 48 小时的账号同属最后一档。
2. **同档中 7 天 Fable 窗口已用最高的越先用**——这些账号的 fable 额度本就用尽，通用额度是
   它们唯一还能贡献的；fable 额度尚有余量的账号留给 fable 请求。fable 请求本身无需特殊处理：
   它的 fable 窗口满了会被下面的跳过规则直接排除。
3. **最久未被分配的优先**——用于打平，让一池全新账号铺开而不是都堆到排序靠前的那个别名上。

**不再按活跃会话数分配**：真正让负载均衡的是窗口用量——某个账号接得多，自己就会填满并排到
后面。会话数只能在两个用量相同的账号之间做选择，而重置时间是更好的依据。

**调度阈值 `MIROFISH_QUOTA_CEILING`（默认 0.90）**：任一相关窗口用到这个比例后，自动分配
就不再选这个账号。这是刻意留出的余量——不是怕额度浪费，而是**反复撞上游的拒绝本身就是
封号诱因**（线上出现过一批账号因「repeated upstream rate-limit refusals」被临时封停）。
可调范围 0.10–1.0，`1.0` 表示一直用到上游自己拒绝为止。先用 0.90 观察，确认没有滥发请求
再往上调。

**阈值不是硬上限**：卡在阈值以上的账号仍然可以被显式指定，而且只有**所有**账号的相关窗口
都真正耗尽（约 100%）时，relay 才本地返回 429。也就是说 90% 阈值不会把还剩一成额度的账号
变成故障——它只影响自动分配的优先级。

缓存最多 10 分钟旧，所以一个读到 89% 的账号可能实际已经过线，代价是多产生一两次 429——
但 429 会被准确识别、按它指明的窗口冷却，冷却期内不再有任何请求，所以代价是有界的。

指定账号的请求不受以上约束，上游 429 仍是最终裁决。

**额度数据不做任何定时轮询**：没有请求时 relay 完全不联系上游。原先每 5 分钟扫描全部账号，
在无人使用的情况下每天也要发约 1.6 万次请求——既没有意义，又正是「持续被拒的流量」这种
最容易触发上游风控的模式（而且连已被封号的账号也照扫）。现在改成：

- **按需**：某个账号服务完一次请求后，若它的额度缓存超过 `MIROFISH_LIMITS_TTL`
  （默认 600 秒）才读一次 `/v1/limits`，且只读这一个账号——同一个账号 10 分钟内最多一次。
  刷新放在请求**之后**，不在前面——放前面会拖慢调用方，也会把上游对这次请求的真实判决盖掉。
- **遇到 429 立即刷新**：这时拒绝本身就证明缓存是错的，拿到新的 `reset_at` 才能让冷却
  时长匹配真实窗口。随后由既有的自动换号逻辑静默重试，调用方无感。
- **手动触发**：面板的「刷新」按钮、保存调度设置时各扫一次。除此之外没有任何定时器。
- 刷新失败完全静默，不影响触发它的请求；已封停、已停用的账号永不探测。

已过重置时间的缓存窗口视为无数据，不会误伤刚刚重置的账号；数据略旧最多让一次请求多试一个
账号——上游 429 加自动换号才是真正的兜底。
