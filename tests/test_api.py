...
async def test_messages_non_stream(client, state, auth_headers):
    ...
    assert upstream_request.headers["x-mirasim-client"] == "0.0.367"
...
async def test_messages_preserves_beta_query_and_claude_fingerprint(
    ...
    # The CLI header carries the same value, paired as the official client
    # pairs them.
...
async def test_count_tokens_proxied(client, state, auth_headers):
    ...
    # Claude Code's own session ids are rewritten per account like any other.
