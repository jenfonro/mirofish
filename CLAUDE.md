...
- Account health: an upstream 401 (credentials/signed session refused) or 503 (no capacity for
  this account) parks the account — `health` in metadata records status, reason, timestamp and a
  ...
  instead). The two refusals differ in whether they heal by themselves, so `HEALTH_RETRY_AFTER`
